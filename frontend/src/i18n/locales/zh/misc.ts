export default {
  ...
  payment: {
    ...
    addon: {
      ...
      guideTitle: '购买前请确认',
      guideSubscription: '如果有多个订阅，请确认选对使用订阅；购买后不能转移到其他订阅。',
      guideUsage: '加油包会在所选订阅的基础额度用尽后自动启用。',
      guideExpiry: '加油包与所选订阅同时到期，未使用额度不会顺延。',
      targetSubscription: '使用订阅',
      expiresAt: '订阅到期：{time}',
      selectPack: '选择加油包',
      unitPrice: '{price} / $1',
      quota: '追加额度',
      validUntil: '有效期至',
      expiryPolicy: '加油包仅用于所选订阅，基础额度用尽后自动启用；未使用额度会随该订阅一同到期。',
      balancePayment: '余额支付',
      balanceSuccess: '加油包已到账',
      buyNow: '购买加油包',
    },
    noPlans: '暂无可用订阅套餐',
    ...
    errors: {
      ...
      ADDON_PURCHASE_DISABLED: '加油包购买暂未开放',
      ADDON_PURCHASE_UNAVAILABLE: '加油包购买服务暂不可用',
      ADDON_PRODUCT_NOT_AVAILABLE: '该加油包已下架或不存在',
      ADDON_PRODUCT_INVALID: '加油包配置无效',
      SUBSCRIPTION_NOT_FOUND: '所选订阅不存在，请刷新后重新选择',
      SUBSCRIPTION_NOT_ACTIVE: '所选订阅即将到期或已失效',
      ADDON_REFUND_NOT_SUPPORTED: '加油包退款需要管理员先核对并撤销额度',
    ...
    admin: {
      ...
      catalogAddons: '加油包商品',
      addonSales: '用户购买加油包',
      addonSalesHint: '关闭后用户购买页不会显示加油包，管理员仍可手动发放。',
      addonSku: 'SKU',
      addonProductName: '商品名称',
      addonQuota: '额度（美元）',
      addonPrice: '售价参数',
      addonUnitPrice: '每 $1 额度单价',
      addonCnyPayPreview: 'CNY 通道',
      addonBalancePayPreview: '余额支付',
      addonBalanceAmount: '余额',
      editAddonProduct: '编辑加油包商品',
      addonPriceChangeNotice: '余额支付按此数值扣除美元余额；CNY 通道在换算汇率为 0 时按同数值收取人民币，配置汇率后按售价乘以汇率。修改只影响新订单，已有订单与额度不变；SKU 不可修改。',
      addonNameRequired: '商品名称不能为空',
      addonQuotaRequired: '额度必须大于 0',
      sortOrderInvalid: '排序必须是大于或等于 0 的整数',
      originalPriceInvalid: '原价必须大于或等于 0',
      paidAt: '支付时间',
      completedAt: '完成时间',
      expiresAt: '过期时间',
      feeRate: '手续费率',
