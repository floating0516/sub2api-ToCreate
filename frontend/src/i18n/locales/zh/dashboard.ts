export default {
  dashboard: {
    ...
    dailyReport: {
      title: '{date} 的 Token 日报',
      open: '查看当天日报',
      todayReport: '查看今日日报',
      loading: '正在生成今天的 Token 日报…',
      retry: '再试一次',
      loadFailed: '日报加载失败，请稍后重试。',
      totalTokens: '今日 Token',
      requests: '请求次数',
      models: '使用模型',
      cacheHitRate: '缓存命中率',
      tokenMix: '输入 {input} · 输出 {output}',
      averageTokens: '单次平均 {value}',
      cacheRead: '缓存读取 {value}',
      topModel: '使用最多：{model}',
      noTopModel: '今天暂无模型使用记录',
      modelLineup: '模型使用明细',
      modelLineupCaption: '按 Token 用量从高到低紧凑排列',
    ...
    overview: {
      ...
      dailyAverageRequests: '日均请求',
      tokensPerRequest: '单次平均 Token',
      currentRPM: '当前 RPM',
      currentTPM: '当前 TPM',
      vsPrevious: '较上一周期',
      noComparison: '暂无对比',
      ...
      heatmapGranularity: '热力图统计维度',
      dailyHeatmap: '每日',
      weeklyHeatmap: '每周',
      cumulativeHeatmap: '累计',
      dailyToken: '每日 Token',
      weeklyToken: '每周 Token',
      cumulativeToken: '累计 Token',
      accountLifetime: '账户全部用量',
      activeDays: '活跃天数',
      tokenUsage: 'Token 用量',
      windowCumulative: '窗口累计',
      yearlyTotal: '近一年累计',
      ...
      dailyAverage: '日均',
      peakDay: '峰值日',
      heatmapPeriod: '热力图时间区间',
      previousPeriod: '查看更早时段',
      nextPeriod: '查看较新时段',
      lessUsage: '较少',
      moreUsage: '较多',
      groupBy: '分组方式',
      groupByModel: '分组方式：模型',
      groupByApiKey: '分组方式：API 密钥',
      ...
      trend: '趋势',
      otherModels: '其他模型',
      platformRanking: '平台用量排行',
      platform: '平台',
      spend: '消费',
      cumulative: '累计数据',
      compositePlatform: '组合平台',
      otherPlatforms: '其他平台',
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      modelsTitle: '这把密钥能打的模型',
      modelsHint: '在 Codex / Claude 里填写下面的模型名。完整列表和定价见「可用渠道」。',
      modelsEmpty: '当前分组还没有可展示的模型。',
      viewChannels: '打开可用渠道',
      openai: {
  ...
  usage: {
    ...
    errors: {
      ...
      hint: '说明',
      originalMessage: '原始报错',
      hints: {
        auth: '认证失败：密钥无效、已停用或复制不完整。请到「API 密钥」页核对。',
        rate_limit: '触发限流：请求太快或上游过载。请稍后再试，或降低并发。',
        quota: '额度不足：余额或订阅额度已用完。请充值或续订后再试。',
        invalid_request: '请求参数有误。请核对模型名、请求格式和必填字段。',
        invalid_request_model: '当前分组不支持这个模型。请到「可用渠道」或密钥页查看能填的模型名。',
        invalid_request_context: '内容太长，超出模型上下文限制。请缩短输入后再试。',
        service_unavailable: '当前没有可用的上游账号。请换一个模型，或稍后再试。',
        upstream: '上游模型服务暂时失败。请稍后重试；若持续出现可换模型或联系管理员。',
        upstream_timeout: '上游响应超时。请稍后重试。',
        internal: '网关内部出错。请稍后重试；若持续出现请联系管理员。',
        cyber: '请求被安全策略拦截。请检查内容是否违规。',
        other: '请求失败。可对照原始报错，或联系管理员。',
      },
      detail: {
        title: '错误请求详情',
        responseBody: '上游响应内容',
  ...
  redeem: {
    ...
    newConcurrency: '新并发数',
    aboutCodes: '关于兑换码',
    codeRule1: '一次性兑换码全局限用一次；活动共享码每个用户限用一次',
