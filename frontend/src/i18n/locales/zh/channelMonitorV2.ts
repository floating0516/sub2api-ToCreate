...
export default {
  channelMonitorV2: {
    title: '渠道监控',
    pageDescription: '上方色块是连通检测，下方表格是真实使用情况，两套数据分开看。',
    usage: {
      title: '使用情况',
      description: '根据真实用户请求汇总成功率和报错，不是探活结果。',
    },
    updating: '正在更新数据',
    updatedTo: '更新至 {time}',
    ...
    bootstrap: {
      title: '正在准备最近监控数据',
      description:
        '首次启用时只聚合最近 24 小时的真实请求，不会往回补 30 天。',
      progress: '进度 {percent}%',
      working: '后台聚合中…',
    },
    timeRange: '时间范围',
    ...
    settings: {
      title: '使用情况',
      description:
        '配置真实用量汇总的维度（平台 / 模型 / 分组）和刷新频率。这些数字只来自用户请求，不会和上方连通检测混成一个分数。',
      save: '保存',
      loading: '加载中...',
      loadFailed: '使用情况配置加载失败',
      saveSuccess: '使用情况配置已保存',
      saveFailed: '使用情况配置保存失败',
      modeBanner:
        '渠道监控已关闭，用量汇总不会运行。配置仍可先保存，开启后立即生效。可在系统设置 → 功能开关调整。',
      modeClosed: '渠道监控已关闭',
      enableTitle: '启用用量汇总',
      enableHint: '关闭后只停止用量汇总；连通检测仍由上方探活列表控制。总开关在「功能开关」。',
      refreshTitle: '汇总频率',
      refreshHint: '影响使用情况表格的刷新节奏',
      refreshAria: '汇总频率',
      ...
      healthTitle: '健康阈值',
      healthHint: '控制使用情况表格里的健康色。默认阈值较宽松，避免少量错误或低缓存率立即显示异常。',
      fields: {
        minimumSample: '最小样本数',
        warningError: '错误率关注 %',
        criticalError: '错误率异常 %',
      ...
      userContract: {
        ...
        trend: '用户端上方色块是连通检测（探活），下方表格才是真实使用情况',
        latency: '延迟展示 AVG · P50 · P90；不展示绝对请求数 / 错误数',
        models: '模型列表留空时展示真实模型名，不会全部归入「其他」',
      },
    },
    admin: {
      description: '连通检测负责探活色块；使用情况负责真实请求汇总。两套数据分开配置，不会合成一个分数。',
      probesTitle: '连通检测',
      probesDescription: '定时向上游发探活，结果出现在用户端色块。改这里会改色块，不会改使用情况表格。',
    },
  },
}
