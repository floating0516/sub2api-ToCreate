<template>
  <button
    ...
    @click="emit('click')"
  >
    <!-- Header: icon + name | probed model + status chip -->
      ...
      <div class="flex-1 min-w-0 grid grid-cols-2 gap-3">
        <div class="min-w-0">
          <div class="text-[10px] font-semibold uppercase tracking-wider text-gray-400">
            {{ t('monitorCommon.channelName') }}
          </div>
          <div class="mt-0.5 text-base font-semibold truncate text-gray-900 dark:text-gray-100">
            {{ item.name }}
          </div>
          <div class="mt-1 flex items-center gap-1.5 min-w-0">
            ...
            <span
              ...
              class="inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-medium bg-gray-100 text-gray-600 dark:bg-dark-700 dark:text-gray-300 truncate"
            >
              {{ item.group_name }}
            </span>
          </div>
        ...
        <div class="min-w-0">
          <div class="text-[10px] font-semibold uppercase tracking-wider text-gray-400">
            {{ t('monitorCommon.probedModel') }}
          </div>
          <div class="mt-0.5 font-mono text-sm font-semibold truncate text-gray-900 dark:text-gray-100">
            {{ item.primary_model }}
          </div>
        </div>
      </div>
