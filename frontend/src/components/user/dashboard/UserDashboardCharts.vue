            ...
            <table class="w-full text-xs">
              ...
              <tbody>
                <tr v-for="(model, index) in models" :key="model.model" class="border-t border-gray-100 dark:border-dark-700">
                  <td class="max-w-[100px] truncate py-1.5 font-medium text-gray-900 dark:text-white" :title="model.model">
                    <span
                      class="mr-1.5 inline-block h-2 w-2 rounded-full align-middle"
                      :style="{ backgroundColor: activityTrendPalette[index % activityTrendPalette.length] }"
                    />
                    {{ model.model }}
                  </td>
...
import { activityTrendPalette } from '@/components/user/dashboard/dashboardActivityTheme'
...
const modelData = computed(() => !props.models?.length ? null : {
  ...
  datasets: [{
    ...
    backgroundColor: props.models.map((_, index) => activityTrendPalette[index % activityTrendPalette.length])
  }]
})
