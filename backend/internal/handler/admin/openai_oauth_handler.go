...
type openAIQuotaHistoryService interface {
	GetQuotaHistory(ctx context.Context, accountID int64, limit int) (*service.OpenAIQuotaHistoryResponse, error)
	SetQuotaHistoryResetSource(ctx context.Context, accountID, cycleID int64, resetSource string) error
	QueryResetCredits(ctx context.Context, accountID int64) (*service.OpenAIResetCredits, error)
}
...
func NewOpenAIOAuthHandler(
	...
	if quotaService != nil {
		...
		h.quotaHistoryService = quotaService
	}
...
func (h *OpenAIOAuthHandler) QueryQuotaHistory(c *gin.Context) {
	...
	if h.quotaHistoryService == nil {
		response.BadRequest(c, "openai quota service is not enabled")
		return
	}
	history, err := h.quotaHistoryService.GetQuotaHistory(c.Request.Context(), accountID, limit)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *OpenAIOAuthHandler) UpdateQuotaHistoryResetSource(c *gin.Context) {
	...
	if err = h.quotaHistoryService.SetQuotaHistoryResetSource(
		c.Request.Context(),
		accountID,
		cycleID,
		req.ResetSource,
...
func (h *OpenAIOAuthHandler) QueryResetCredits(c *gin.Context) {
	...
	credits, err := h.quotaHistoryService.QueryResetCredits(c.Request.Context(), accountID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
