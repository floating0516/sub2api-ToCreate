...
func RegisterUserRoutes(
	...
	authenticated.Use(gin.HandlerFunc(auditLog))
	{
		...
		user := authenticated.Group("/user")
		{
			...
			user.GET("/check-in/status", h.Checkin.GetStatus)
			user.POST("/check-in", h.Checkin.CheckIn)
			user.GET("/check-in/history", h.Checkin.History)
		...
		usage.Use(panelRateLimiter.Heavy())
		{
			...
			usage.GET("/daily-report", h.Usage.DailyReport)
			usage.GET("/:id", h.Usage.GetByID)
			usage.GET("/stats", h.Usage.Stats)
			// User dashboard endpoints
			usage.GET("/dashboard/stats", h.Usage.DashboardStats)
			usage.GET("/dashboard/trend", h.Usage.DashboardTrend)
			usage.GET("/dashboard/model-trends", h.Usage.DashboardModelTrends)
		...
		subscriptions := authenticated.Group("/subscriptions")
		{
			...
			subscriptions.GET("/:id/usage-timeline", h.Subscription.GetUsageTimeline)
		}
