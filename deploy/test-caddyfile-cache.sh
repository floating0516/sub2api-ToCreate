...
for sensitive_query_key in code state nonce code_verifier code_challenge id_token access_token client_secret token; do
	if ! printf '%s\n' "$normalized_config" | grep -Eq "^replace $sensitive_query_key REDACTED([[:space:]]|$)"; then
		echo "Caddyfile must redact query key: $sensitive_query_key" >&2
		exit 1
	fi
done

echo "Caddyfile preserves backend cache policy, SSE streaming, non-SSE compression, and OIDC query redaction"
